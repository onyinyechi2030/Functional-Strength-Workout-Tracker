...
function recordTrainingMinutes(r){
 ...
 if(!isRecovery)return {strength:+r?.totalMinutes||0,recovery:0};
 const activityMinutes=(r?.exercises||[]).reduce((sum,e)=>{
   if(!e?.recovery)return sum;
   const m=parseFloat(e?.setData?.[0]?.minutes);
   return sum+(Number.isFinite(m)?m:0);
 },0);
 return {strength:0,recovery:activityMinutes||(+r?.totalMinutes||0)};
}
...
function renderDashboard(){
 const all=records(),today=isoToday(),ws=weekStart(today),we=addDays(ws,6),month=monthKey(today),weekRecords=all.filter(r=>r.date>=ws&&r.date<=we),monthRecords=all.filter(r=>monthKey(r.date)===month),st=streakStats();
 const currentMins=weekRecords.map(recordTrainingMinutes);
 const strengthMinutes=currentMins.reduce((n,m)=>n+m.strength,0);
 const otherMinutes=currentMins.reduce((n,m)=>n+m.recovery,0);
