...
const dashboardWeekEl=()=>el('dashboardWeek');
...
function updateDateDayNotice(){const d=PROGRAM[dayIndexFromDate(workoutDate.value)];const n=document.getElementById('dateDayNotice');if(n)n.textContent=`Selected date is ${d.day}. The workout shown is linked automatically to this date.`;}
function renderDaybar(){updateDateDayNotice();daybar.innerHTML=PROGRAM.map((d,i)=>`<button class="daybtn ${i===activeDay?'active':''}" data-i="${i}"><b>${d.day}</b><span>${d.time} · ${d.ex.length} ${d.kind==='recovery'?'activities':'exercises'}</span></button>`).join('');$$('.daybtn').forEach(b=>b.onclick=()=>{const next=+b.dataset.i;workoutDate.value=dateForDayInSameWeek(workoutDate.value,next);activeDay=next;renderWorkout(true);updateDateDayNotice()});}
...
function weeklyTrainingTimeRows(limit=12){
 ...
 for(let i=0;i<limit;i++){
   ...
   const total=weekRecords.reduce((n,r)=>n+(+r.totalMinutes||0),0);
   const strength=weekRecords.filter(r=>r.kind!=='recovery').reduce((n,r)=>n+(+r.totalMinutes||0),0);
   const recovery=weekRecords.filter(r=>r.kind==='recovery').reduce((n,r)=>n+(+r.totalMinutes||0),0);
   rows.push({start,end,total,strength,recovery,current:i===0,hasData:weekRecords.length>0});
 }
 return rows;
}
function renderWeeklyTrainingTimeHistory(){
 const rows=weeklyTrainingTimeRows(12);
 weeklyTimeHistoryEl().innerHTML=`<div class="weekly-time-list">${rows.map(r=>`<div class="weekly-time-row ${r.current?'current':''}"><div class="weekly-time-dates"><b>${r.current?'Current week':'Week of '+r.start}</b><small>${r.start} – ${r.end}</small></div><div class="weekly-time-breakdown">${r.hasData?`<div><span>Strength</span><b>${r.strength} min</b></div><div><span>Other / recovery</span><b>${r.recovery} min</b></div>`:`<small>No sessions logged</small>`}</div></div>`).join('')}</div>`;
}

function renderDashboard(){
 const all=records(),today=isoToday(),ws=weekStart(today),we=addDays(ws,6),month=monthKey(today),weekRecords=all.filter(r=>r.date>=ws&&r.date<=we),monthRecords=all.filter(r=>monthKey(r.date)===month),st=streakStats(),strengthMinutes=weekRecords.filter(r=>r.kind!=='recovery').reduce((n,r)=>n+(+r.totalMinutes||0),0),otherMinutes=weekRecords.filter(r=>r.kind==='recovery').reduce((n,r)=>n+(+r.totalMinutes||0),0);
 ...
 const wp=aggregate(weekRecords),mp=aggregate(monthRecords),weekExtra=weekRecords.reduce((n,r)=>n+additionalWork(r).done,0),monthExtra=monthRecords.reduce((n,r)=>n+additionalWork(r).done,0);
 ...
 dashboardWeekEl().innerHTML=days.map((day,i)=>{const d=new Date(ws+'T12:00:00');d.setDate(d.getDate()+i);const ds=d.toISOString().slice(0,10),rec=all.find(r=>r.date===ds&&r.day===day),p=rec?workoutProgress(rec):null;return `<div class="week-cell ${p&&p.percent===100?'done':''}"><b>${day.slice(0,3)}</b><small>${ds.slice(5)}</small><div>${p?p.percent+'%':'—'}</div>${p?`<small>${p.done}/${p.total} sets</small>`:''}</div>`}).join('');
 renderWeeklyTrainingTimeHistory();
 const prs=personalBests();dashboardPRsEl().innerHTML=prs.length?prs.map(p=>`<div class="pr-card"><b>${esc(p.name)}</b><span>${p.w?p.w+' lb':p.t?p.t+' sec':p.rep?p.rep+' reps':'Recorded'} · ${p.date}</span></div>`).join(''):'<div class="empty">Personal bests will appear after performance data are saved.</div>'
}
...
function updateCompletion(){const p=programProgress({exercises:currentExercises}),extra=additionalWork({exercises:currentExercises}),primary=primaryProgress({exercises:currentExercises}),isRecovery=PROGRAM[activeDay].kind==='recovery';completionBar.style.width=p.percent+'%';completionText.textContent=isRecovery?`${p.done} of ${p.total} planned recovery activities completed (${p.percent}%). Optional activities add information without lowering the percentage.`:`Program completion: ${p.done} of ${p.total} prescribed sets (${p.percent}%).${primary.total?` Primary work: ${primary.done}/${primary.total} sets (${primary.percent}%).`:''}${extra.done?` Additional work: ${extra.done} extra set${extra.done===1?'':'s'} performed.`:''}`;if(window.topCompletionBar)topCompletionBar.style.width=p.percent+'%';if(window.topCompletionText)topCompletionText.textContent=isRecovery?`${p.done} of ${p.total} activities · ${p.percent}%`:`Program ${p.done}/${p.total} · ${p.percent}%${primary.total?` · Primary ${primary.percent}%`:''}${extra.done?` · +${extra.done} extra`:''}`;document.querySelectorAll('.exercise').forEach((card,i)=>{const ep=exerciseProgress(currentExercises[i]);const label=card.querySelector('.exercise-progress');if(label)label.textContent=currentExercises[i].recovery?`${ep.percent}% complete`:`${ep.done}/${ep.total} sets · ${ep.percent}%`;card.classList.toggle('done',ep.percent===100)})}
...
function collect(){const d=PROGRAM[activeDay],date=workoutDate.value;return{id:recordId(d.day,date),date,day:d.day,kind:d.kind,focus:d.focus,plannedCount:d.ex.length,totalMinutes:totalMinutes.value,rpe:rpe.value,energyBefore:energyBefore.value,energyAfter:energyAfter.value,notes:notes.value,updatedAt:new Date().toISOString(),exercises:currentExercises.map(e=>({...e,setData:e.setData.map(s=>({...s}))}))}}
let autoSaveTimer=null;function meaningfulCurrentSession(){return currentExercises.some(e=>e.complete||e.selectedType||(e.setData||[]).some(setHasData))||totalMinutes.value||energyBefore.value||energyAfter.value||notes.value.trim()}
function saveSilently(){if(!workoutDate.value||!meaningfulCurrentSession())return null;const rec=collect(),all=records(),idx=all.findIndex(x=>x.id===rec.id);if(idx>=0)all[idx]=rec;else all.push(rec);setRecords(all);renderDashboard();return rec}
function scheduleAutoSave(){clearTimeout(autoSaveTimer);autoSaveTimer=setTimeout(()=>{const rec=saveSilently();if(rec){completionSummary.textContent='Saved automatically';completionSummary.classList.add('show');setTimeout(()=>completionSummary.classList.remove('show'),1400)}},650)}
...
function renderHistory(){const r=filtered(),done=r.reduce((n,x)=>n+workoutProgress(x).done,0),planned=r.reduce((n,x)=>n+workoutProgress(x).total,0),avg=planned?Math.round(done/planned*100):0,minutes=r.reduce((n,x)=>n+(+x.totalMinutes||0),0);const extraSets=r.reduce((n,x)=>n+additionalWork(x).done,0);historyStats.innerHTML=`<div class="stat"><span>Sessions logged</span><b>${r.length}</b></div><div class="stat"><span>Program completion</span><b>${avg}%</b></div><div class="stat"><span>Prescribed sets</span><b>${done}/${planned}</b></div><div class="stat"><span>Additional work</span><b>${extraSets} sets</b></div><div class="stat"><span>Total minutes</span><b>${minutes}</b></div>`;historyList.innerHTML=r.length?r.map(x=>{const p=workoutProgress(x),extra=additionalWork(x),isRecovery=x.kind==='recovery'||x.day==='Thursday'||x.day==='Sunday',performed=x.exercises.filter(e=>exerciseProgress(e).done>0);return `<div class="history-item"><div class="history-head"><div><h3>${esc(x.day)} · ${x.date}</h3><div class="history-meta">${esc(x.focus)} · ${p.done}/${p.total} ${isRecovery?'activities':'prescribed sets'} (${p.percent}%)${!isRecovery&&extra.done?' · +'+extra.done+' extra sets':''}${x.totalMinutes?' · '+x.totalMinutes+' min':''}${x.rpe?' · RPE '+x.rpe:''}</div></div><div class="history-actions"><button class="small" onclick="openRecord('${x.id}')">Open</button><button class="small danger" onclick="deleteRecord('${x.id}')">Delete</button></div></div><div class="completed-list">${performed.map(e=>{const ep=exerciseProgress(e);return `<div><b>${esc(displayExerciseName(e))}</b> — ${ep.done}/${ep.total} ${e.recovery?'activity':'sets'} (${ep.percent}%) · ${bestSummary(e)}</div>`}).join('')||'<div>No completed sets recorded.</div>'}</div>${x.notes?`<p><b>Notes:</b> ${esc(x.notes)}</p>`:''}</div>`}).join(''):'<div class="card empty">No workouts match these filters.</div>'}
...
function showSimpleSummary(){
  ...
  completionSummary.innerHTML=`<b>${PROGRAM[activeDay].kind==='recovery'?'Recovery session':'Workout'} saved.</b> ${p.done} of ${p.total} ${PROGRAM[activeDay].kind==='recovery'?'activities':'prescribed sets'} completed (${p.percent}%). ${primary.total?`Primary work: ${primary.done}/${primary.total} (${primary.percent}%). `:''}${extra.done?`Additional work: ${extra.done} extra set${extra.done===1?'':'s'}. `:''}${rec.totalMinutes?`${rec.totalMinutes} minutes recorded. `:''}${volume?`${volume.toLocaleString()} total logged volume.`:''}`;
