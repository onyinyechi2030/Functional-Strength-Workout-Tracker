...
function recordTrainingMinutes(r){
 ...
 if(!isRecovery){
   ...
   return {strength:Number.isFinite(m)?m:0,recovery:0};
 }
 ...
 const fallback=parseFloat(r?.totalMinutes);
 return {strength:0,recovery:activityMinutes||(Number.isFinite(fallback)?fallback:0)};
}
...
function renderDashboard(){
 try{
   const all=records();
   const today=isoToday();
   const ws=weekStart(today),we=addDays(ws,6),month=monthKey(today);
   const weekRecords=all.filter(r=>r&&r.date>=ws&&r.date<=we);
   const monthRecords=all.filter(r=>r&&monthKey(r.date||'')===month);
   ...
   if(status){
     const dated=weekRecords.map(r=>`${r.day||''} ${r.date}: ${r.totalMinutes||0} min`).join(' · ');
     status.textContent=weekRecords.length
       ? `${weekRecords.length} saved session${weekRecords.length===1?'':'s'} found this week.${dated?` ${dated}`:''}`
       : `No saved sessions found between ${ws} and ${we}.`;
   }
   renderWeeklyTrainingTimeHistory();
   const prs=personalBests();
   const prBox=dashboardPRsEl();
   if(prBox){
     prBox.innerHTML=prs.length
       ? prs.map(p=>`<div class="pr-card"><b>${esc(p.name)}</b><span>${p.w?p.w+' lb':p.t?p.t+' sec':p.rep?p.rep+' reps':'Recorded'} · ${p.date}</span></div>`).join('')
       : '<div class="empty">Personal bests will appear after performance data are saved.</div>';
   }
 }catch(err){
   console.error('Dashboard render failed',err);
   const status=document.getElementById('dashboardDataStatus');
   if(status) status.textContent=`Dashboard error: ${err.message}`;
 }
}
...
function init(){
 ...
 try{
   ...
   bindClick('resetFilters',()=>{
     ...
     if(f)f.value=''; if(t)t.value=''; if(d)d.value=''; if(p)p.value='all'; renderHistory();
   });
