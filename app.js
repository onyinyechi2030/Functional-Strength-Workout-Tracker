...
function programVersionLabel(date){
  ...
  return 'Four-day program · Walking Lunge update effective August 19, 2026';
}
...
function normalizeActivities(r){
  ...
  return {
    ...
    breathing:Array.isArray(a.breathing)?a.breathing:[],
    meditation:Array.isArray(a.meditation)?a.meditation:[]
  };
}
function yogaTypes(){
...
function practiceSessionHTML(kind,session={}){
 const types=kind==='breathing'?breathingTypes():meditationTypes();
 const def=kind==='breathing'?'Box Breathing':'Mindfulness Meditation';
 const opts=types.map(x=>`<option value="${esc(x)}" ${(session.type||def)===x?'selected':''}>${esc(x)}</option>`).join('');
 return `<div class="practice-session" data-practice-kind="${kind}">
   <label>${kind==='breathing'?'Breathing type':'Meditation type'}<select class="practiceType">${opts}</select></label>
   <label>Duration<select class="practiceMinutes">${activityMinuteOptions(session.minutes||'')}</select></label>
   <button type="button" class="secondary remove-practice">Remove</button>
   <label class="practice-notes">Notes<textarea class="practiceNotes" placeholder="Optional notes">${esc(session.notes||'')}</textarea></label>
 </div>`;
}
function readPracticeUI(){
 const out={breathing:[],meditation:[]};
 document.querySelectorAll('.practice-session').forEach(row=>{
   const kind=row.dataset.practiceKind;
   const type=row.querySelector('.practiceType')?.value||'';
   const minutes=parseFloat(row.querySelector('.practiceMinutes')?.value)||0;
   const notes=row.querySelector('.practiceNotes')?.value||'';
   if((kind==='breathing'||kind==='meditation')&&(minutes>0||notes.trim()))out[kind].push({type,minutes,notes});
 });
 return out;
}
function updateDailyPracticeTotals(){
 const p=readPracticeUI();
 if(dailyBreathingTotalEl())dailyBreathingTotalEl().textContent=`${p.breathing.reduce((n,s)=>n+(Number(s.minutes)||0),0)} min`;
 if(dailyMeditationTotalEl())dailyMeditationTotalEl().textContent=`${p.meditation.reduce((n,s)=>n+(Number(s.minutes)||0),0)} min`;
}
function bindPracticeRows(){
 document.querySelectorAll('.practice-session').forEach(row=>{
   row.querySelectorAll('select,textarea').forEach(node=>{
     node.addEventListener('change',()=>{updateDailyPracticeTotals();scheduleAutoSave()});
     if(node.tagName==='TEXTAREA')node.addEventListener('input',scheduleAutoSave);
   });
   const btn=row.querySelector('.remove-practice');
   if(btn)btn.onclick=()=>{row.remove();updateDailyPracticeTotals();scheduleAutoSave()};
 });
}
function renderDailyPractices(rec=null){
 const a=normalizeActivities(rec);
 if(breathingSessionsEl())breathingSessionsEl().innerHTML=a.breathing.map(s=>practiceSessionHTML('breathing',s)).join('');
...
function addPracticeSession(kind){
 const host=kind==='breathing'?breathingSessionsEl():meditationSessionsEl();
 if(!host)return;
 host.insertAdjacentHTML('beforeend',practiceSessionHTML(kind,{}));
 bindPracticeRows();updateDailyPracticeTotals();
}
...
function weeklyTrainingTimeRows(count=12){
 ...
 for(let i=0;i<count;i++){
   ...
   rows.push({
     ...
     breathing:rs.reduce((n,r)=>n+breathingMinutesForRecord(r),0),
     meditation:rs.reduce((n,r)=>n+meditationMinutesForRecord(r),0),
     hasData:rs.some(r=>(parseFloat(r?.totalMinutes)||0)>0||recordYogaMinutes(r)>0||recordWalkingMinutes(r)>0||breathingMinutesForRecord(r)>0||meditationMinutesForRecord(r)>0||workoutProgress(r).done>0)
   });
 }
 return rows;
}
function renderWeeklyTrainingTimeHistory(){
 ...
 box.innerHTML=`<div class="weekly-time-list">${rows.map(r=>`<div class="weekly-time-row ${r.current?'current':''}"><div class="weekly-time-dates"><b>${r.current?'Current week':'Week of '+r.start}</b><small>${r.start} – ${r.end}</small></div><div class="weekly-time-breakdown">${r.hasData?`<div><span>Strength</span><b>${r.strength} min</b></div><div><span>Yoga</span><b>${r.yoga} min</b></div><div><span>Walking</span><b>${r.walking} min</b></div><div><span>Breathing</span><b>${r.breathing} min</b></div><div><span>Meditation</span><b>${r.meditation} min</b></div>`:`<small>No activity logged</small>`}</div></div>`).join('')}</div>`;
}
function renderDashboard(){
  ...
  try{
    ...
    const breathingMinutes=weekRecords.reduce((n,r)=>n+breathingMinutesForRecord(r),0);
    const meditationMinutes=weekRecords.reduce((n,r)=>n+meditationMinutesForRecord(r),0);
    ...
    setText('dashBreathingMinutes',`${breathingMinutes} min`);
    setText('dashMeditationMinutes',`${meditationMinutes} min`);

    if(status){
      const details=weekRecords.map(r=>{
        const sm=r.kind==='recovery'?0:(parseFloat(r.totalMinutes)||0);
        const ym=recordYogaMinutes(r),wm=recordWalkingMinutes(r),bm=breathingMinutesForRecord(r),mm=meditationMinutesForRecord(r);
        const bits=[];
        if(sm)bits.push(`Strength ${sm}`);
        if(ym)bits.push(`Yoga ${ym}`);
        if(wm)bits.push(`Walking ${wm}`);
        if(bm)bits.push(`Breathing ${bm}`);
        if(mm)bits.push(`Meditation ${mm}`);
        return `${r.day||''} ${r.date}${bits.length?`: ${bits.join(', ')} min`:''}`;
      }).join(' · ');
...
let autoSaveTimer=null;function meaningfulCurrentSession(){
 ...
 const hasActivity=a.yoga.length>0||a.walking.length>0||p.breathing.length>0||p.meditation.length>0;
...
function renderHistory(){
 ...
 historyStatsEl().innerHTML=`<div class="stat"><span>Days logged</span><b>${list.length}</b></div><div class="stat"><span>Strength</span><b>${strength} min</b></div><div class="stat"><span>Yoga</span><b>${yoga} min</b></div><div class="stat"><span>Walking</span><b>${walking} min</b></div><div class="stat"><span>Breathing</span><b>${breathing} min</b></div><div class="stat"><span>Meditation</span><b>${meditation} min</b></div><div class="stat"><span>Program completion</span><b>${avg}%</b></div>`;
 historyListEl().innerHTML=list.length?list.map(r=>{
   ...
   return `<div class="history-item">
    <div class="history-head"><div><h3>${esc(r.day)} · ${r.date}</h3><div class="history-meta">${sm?`Strength ${sm} min`:''}${ym?`${sm?' · ':''}Yoga ${ym} min`:''}${wm?`${sm||ym?' · ':''}Walking ${wm} min`:''}${bm?`${sm||ym||wm?' · ':''}Breathing ${bm} min`:''}${mm?`${sm||ym||wm||bm?' · ':''}Meditation ${mm} min`:''}</div></div><div class="history-actions"><button class="small" onclick="openRecord('${r.id}')">Open</button><button class="small danger" onclick="deleteRecord('${r.id}')">Delete</button></div></div>
...
function init(){
 ...
 try{
   ...
   bindClick('saveWorkout',save);bindClick('addYogaSession',()=>addActivitySession('yoga'));bindClick('addWalkingSession',()=>addActivitySession('walking'));bindClick('addBreathingSession',()=>addPracticeSession('breathing'));bindClick('addMeditationSession',()=>addPracticeSession('meditation'));
